 ...
 */
/**
 * ImuOCf stands for inertial movement unit orientation complementary filter. Quaternion is added
 * because the filter applies the complementary filter to the quaternions from the gyroscope and
 * acceleration/magnetic sensors, respectively.
 *
 * <p>The complementary filter is a frequency domain filter. In its strictest sense, the definition
 * of a complementary filter refers to the use of two or more transfer functions, which are
 * mathematical complements of one another. Thus, if the data from one sensor is operated on by
 * G(s), then the data from the other sensor is operated on by I-G(s), and the sum of the transfer
 * functions is I, the identity matrix.
 *
 * <p>ImuOCfQuaternion attempts to fuse magnetometer, gravity and gyroscope sensors together to
 * produce an accurate measurement of the rotation of the device.
 *
 * <p>The magnetometer and acceleration sensors are used to determine one of the two orientation
 * estimations of the device. This measurement is subject to the constraint that the device must not
 * be accelerating and hard and soft-iron distortions are not present in the local magnetic field..
 *
 * <p>The gyroscope is used to determine the second of two orientation estimations of the device.
 * The gyroscope can have a shorter response time and is not effected by linear acceleration or
 * magnetic field distortions, however it experiences drift and has to be compensated periodically
 * by the acceleration/magnetic sensors to remain accurate.
 *
 * <p>Quaternions are used to integrate the measurements of the gyroscope and apply the rotations to
 * each sensors measurements via complementary filter. This the ideal method because quaternions are
 * not subject to many of the singularties of rotation matrices, such as gimbal lock.
 *
 * <p>The quaternion for the magnetic/acceleration sensor is only needed to apply the weighted
 * quaternion to the gyroscopes weighted quaternion via complementary filter to produce the fused
 * rotation. No integrations are required.
 *
 * <p>The gyroscope provides the angular rotation speeds for all three axes. To find the orientation
 * of the device, the rotation speeds must be integrated over time. This can be accomplished by
 * multiplying the angular speeds by the time intervals between sensor updates. The calculation
 * produces the rotation increment. Integrating these values again produces the absolute orientation
 * of the device. Small errors are produced at each iteration causing the gyro to drift away from
 * the true orientation.
 *
 * <p>To eliminate both the drift and noise from the orientation, the gyroscope measurements are
 * applied only for orientation changes in short time intervals. The magnetometer/acceleration
 * fusion is used for long time intervals. This is equivalent to low-pass filtering of the
 * accelerometer and magnetic field sensor signals and high-pass filtering of the gyroscope signals.
 */
/** !! dummy code !! */
public class ImuOCfQuaternion extends Orientation {
  ...
  private float[] originAccel = new float[3];
  private float[] originMag = new float[3];
  private float[] originGyro = new float[3];

  private Quaternion quatGyroDelta;
  private Quaternion quatGyro;
  private Quaternion quatAccelMag;

  public ImuOCfQuaternion() {
    super();
    // default constructor
  }

  /**
   * Get the orientation of the device. This method can be called *only* after setAcceleration(),
   * setMagnetic() and getGyroscope() have been called.
   *
   * @return float[] an array containing the linear acceleration of the device where values[0]:
   *     azimuth, rotation around the Z axis. values[1]: pitch, rotation around the X axis.
   *     values[2]: roll, rotation around the Y axis. with respect to the Android coordinate system.
   */
  ...
  /**
   * The complementary filter coefficient, a floating point value between 0-1, exclusive of 0,
   * inclusive of 1.
   *
  ...
  public void setMagneticField(float[] values) {
    originMag = values;
    if (originAccel[2] != 0) {
      getRotationVectorFromAccelMag(fusion(originAccel, values));
    }
  }
  ...
  public void setAccelerometer(float[] values) {
    originAccel = values;
    if (originMag[2] != 0) {
      getRotationVectorFromAccelMag(fusion(originMag, values));
    }
  }

  @Override
  public void setGyroscope(float[] values, long eventTime) {
    originGyro = values;
    getRotationVectorFromGyro();
  }

  private double[] convert(float[] floatArray) {
  ...
  private float[] fusion(float[] accel, float[] mag) {
    float oneMinusCoeff = 1.0f - filterCoefficient;
    float[] fusion = new float[accel.length];
    fusion[0] = filterCoefficient * mag[0] + oneMinusCoeff * accel[0];
    fusion[1] = filterCoefficient * mag[1] + oneMinusCoeff * accel[1];
    fusion[2] = filterCoefficient * mag[2] + oneMinusCoeff * accel[2];
    return fusion;
  }
  ...
  /**
   * Create an angle-axis vector, in this case a unit quaternion, from the provided Euler angle's
   * (presumably from SensorManager.getOrientation()).
   *
   * <p>Equation from http://www.euclideanspace.com/maths/geometry/rotations/conversions
   * /eulerToQuaternion/
   *
   * @param orientation
   */
  ...
  private void getRotationVectorFromGyro() {
    ...
    float magnitude =
        (float)
            Math.sqrt(
                Math.pow(vGyroscope[0], 2)
                    + Math.pow(vGyroscope[1], 2)
                    + Math.pow(vGyroscope[2], 2));
